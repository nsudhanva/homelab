...
class EmailClassifier:
    ...
    def _build_prompt(
        ...
        return (
            ...
            "1. PRIMARY INTENT OVER INCIDENTAL KEYWORDS:\n"
            "   - If a recruiter or agency (e.g. Lumicity, Rocket) reaches out about a job or "
            "interview, the intent is RECRUITMENT ('Companies/Jobs'). Even if they mention past "
            "work at another company (e.g. Montai, Google, Autodesk), do NOT classify under the "
            "past employer's company label.\n"
            "   - If an email is an application confirmation/status for a role applied to, use "
            "'Companies/Apply'.\n"
            "   - If a merchant/subscription mentions a card/payment method (e.g. Amex, Chase), "
            "classify by the merchant/service or personal category, NOT the credit card name.\n"
            "   - Dedicated financial labels ('Personal/Amex', 'Personal/Banks') are "
            "ONLY for emails sent directly by American Express or banks regarding "
            "statements, card alerts, or fraud.\n"
            "2. HIERARCHICAL SPECIFICITY:\n"
            "   - Always prefer specific child labels (e.g. 'Personal/Amazon', 'Personal/Apple') "
            "over generic parent labels ('Companies', 'Personal') when it directly pertains to "
            "that service.\n"
            "3. MANDATORY OUTPUT:\n"
            "   - Pick the single best-fitting label from the Active Allowed Labels above.\n"
            "   - If none fit the primary intent or you are uncertain, select 'QUARANTINE'.\n"
